...
public class JwtHeaderAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (SecurityContextHolder.getContext().getAuthentication() == null) {
            Optional<AdminUserPrincipal> fromGatewayHeaders = principalFromGatewayHeaders(request);
            Optional<AdminUserPrincipal> fromBearerToken = fromGatewayHeaders.isPresent()
                    ? Optional.empty()
    ...
    private Optional<AdminUserPrincipal> principalFromGatewayHeaders(HttpServletRequest request) {
        ...
        try {
            AdminUserPrincipal principal = new AdminUserPrincipal(Long.parseLong(userId), email, resolvedRole);
            log.debug("Authenticated user from gateway headers: {} ({}) with role: {}", email, userId, resolvedRole);
    ...
    private Optional<AdminUserPrincipal> principalFromBearerToken(HttpServletRequest request) {
        ...
        String resolvedRole = StringUtils.hasText(role) ? role : "CUSTOMER";
        AdminUserPrincipal principal = new AdminUserPrincipal(userId, email, resolvedRole);
        log.debug("Authenticated user from bearer token: {} ({}) with role: {}", email, userId, resolvedRole);
        return Optional.of(principal);
    }
}
